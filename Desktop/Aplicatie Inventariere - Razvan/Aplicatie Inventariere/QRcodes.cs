...
namespace Aplicatie_Inventariere
{
    public partial class QRcodes : Form
    {
        ...
        public QRcodes()
        {
            ...
            LocalDataStorage.email = "razviradulescu@gmail.com";
        ...
        private void QRcodes_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Could not get printing queue, " + err.Message);
            }
        }

        private void getDataFromDatabase()
        {
            FirebaseResponse response = client.Get(@"PendingForPrint/" + LocalDataStorage.email.Replace('.', ','));
            var result = response.ResultAs<Dictionary<string, ObjectDto>>();
            foreach(var obj in result)
            {
                allPendingCodes.Add(new QRCode(obj.Key, obj.Value.Name, obj.Value.Room));
            }
        }
        ...
        private void createQrBtn_Click(object sender, EventArgs e)
        {
            ...
            string path = @"PendingForPrint/" + LocalDataStorage.email.Replace('.', ',') + "/" + UUID;
        ...
        private void deleteItems(IEnumerable<QRCode> qrCodesToDelete)
        {
            ...
            foreach(QRCode code in listToDelete)
            {
                try
                {
                    client.Delete(@"PendingForPrint/" + LocalDataStorage.email.Replace('.', ',') + "/" + code.UUID);
                }
                catch { }
                string path = @"Objects/" + LocalDataStorage.email.Replace('.', ',') + "/" + code.UUID;
                client.Set(path, new ObjectDto(code.Name, code.Room));
